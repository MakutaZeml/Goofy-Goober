...
public class KosmXShieldBubble extends KosmXWindupAttackHandler {
    public KosmXShieldBubble(ResourceLocation id) {
        super(id);
    }
    private static final ResourceLocation SHIELD = new ResourceLocation(ExtraHamonStandsAddon.MOD_ID, "bubble_shield");


    @Override
    public boolean setWindupAnim(PlayerEntity player) {
        return setAnimFromName(player, SHIELD, anim-> new KosmXShieldBubble.ChargedAttackAnimPlayer(anim).windupStopsAt(anim.returnToTick));
    }

    @Override
    public boolean setAttackAnim(PlayerEntity player) {
        return setToSwingTick(player, -1, SHIELD);
    }
